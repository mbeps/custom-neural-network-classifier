...
class NeuralNet:
    ...
    def _init_parameters(self) -> None:
        ...
        for out_dim in self.hidden_sizes:
            w: np.ndarray = np.random.randn(in_dim, out_dim).astype(
                np.float32
            ) * np.sqrt(2.0 / in_dim)
        ...
        w_out: np.ndarray = np.random.randn(in_dim, self.output_size).astype(
            np.float32
        ) * np.sqrt(2.0 / in_dim)
    ...
    def fit(self, X: np.ndarray, y_integer: np.ndarray) -> None:
        ...
        for epoch in range(self.epochs):
            ...
            if self.descent_type == "batch" or self.batch_size is None:
                ...
                gradients: Dict[str, List[Optional[np.ndarray]]] = self._backward_pass(
            ...
            print(
                f"Epoch {epoch + 1}/{self.epochs}\tLoss: {epoch_loss:.4f}\tAccuracy: {accuracy * 100:.2f}%"
            )
    ...
    def _forward_pass(
        ...
        cache["output"] = {"z_out": z_out, "probs": probs}
        return activations, cache, probs

    def _compute_loss(
        self, probs: np.ndarray, y_integer: np.ndarray
    ) -> Tuple[float, float]:
        """
        ...
        """
        correct_logprobs: np.ndarray = -np.log(
            probs[np.arange(probs.shape[0]), y_integer] + 1e-8
        )
    ...
    def _backward_pass(
        ...
        if self.num_hidden_layers > 0:
            last_hidden_activation: np.ndarray = cache[
                f"layer_{self.num_hidden_layers - 1}"
            ]["activation"]
        else:
            last_hidden_activation = X

        dW[-1] = (
            np.dot(last_hidden_activation.T, d_out) + self.l2_lambda * self.weights[-1]
        )
        ...
        for i in reversed(range(self.num_hidden_layers)):
            ...
            if self.use_batchnorm:
                ...
                d_var: np.ndarray = np.sum(
                    d_z_hat * (z - mu) * -0.5 * (var + eps) ** -1.5,
                    axis=0,
                    keepdims=True,
                )
                d_mu: np.ndarray = np.sum(
                    d_z_hat * (-1.0 / np.sqrt(var + eps)), axis=0, keepdims=True
                ) + d_var * np.mean(-2.0 * (z - mu), axis=0, keepdims=True)
                dz: np.ndarray = (
                    (d_z_hat / np.sqrt(var + eps))
                    + (d_var * 2.0 * (z - mu) / N)
                    + (d_mu / N)
                )
    ...
    def _update_parameters(
        self, grads: Dict[str, List[Optional[np.ndarray]]], current_lr: float
    ) -> None:
        """
        ...
        for i, dw_ in enumerate(grads["dW"]):
            if dw_ is not None:
                ...
                self.velocity_w[i] = (
                    self.momentum * self.velocity_w[i] + current_lr * grad_dw
                )
        ...
        for i, db_ in enumerate(grads["db"]):
            if db_ is not None:
                ...
                self.velocity_b[i] = (
                    self.momentum * self.velocity_b[i] + current_lr * grad_db
                )
        ...
        if self.use_batchnorm:
            for i, dgamma_ in enumerate(grads["dgamma"]):
                if dgamma_ is not None:
                    ...
                    if self.use_grad_clip:
                        ...
                        if grad_norm > self.grad_clip_norm:
                            grad_dgamma = grad_dgamma * (
                                self.grad_clip_norm / grad_norm
                            )
                    self.velocity_gamma[i] = (
                        self.momentum * self.velocity_gamma[i]
                        + current_lr * grad_dgamma
                    )
            ...
            for i, dbeta_ in enumerate(grads["dbeta"]):
                if dbeta_ is not None:
                    ...
                    self.velocity_beta[i] = (
                        self.momentum * self.velocity_beta[i] + current_lr * grad_dbeta
                    )
    ...
    def _save_current_weights(self) -> None:
        ...
        if self.use_batchnorm:
            ...
            self.best_weights["velocity_gamma"] = [
                vg.copy() for vg in self.velocity_gamma
            ]
            self.best_weights["velocity_beta"] = [
                vb.copy() for vb in self.velocity_beta
            ]
...
class Classifier:
    ...
    def __init__(self) -> None:
        ...
        print(
            f"Classifier initialised with hidden_layers={self.hidden_layers}, learning_rate={self.learning_rate}"
        )
    ...
    def fit(self, data: Any, target: Any) -> None:
        ...
        if self.enable_grid_search and X_val.shape[0] > 0:
            best_params: Dict[str, Any] = self._grid_search_hyperparams(
    ...
    def _init_nn(
        ...
        print(
            f"Initialising NeuralNet with hidden_layers={hidden_layers}, dropout_rate={dropout_rate}"
        )
        self.nn = NeuralNet(
    ...
    def _grid_search_hyperparams(
        ...
        for layers in layer_configs:
            for dr in dropout_rates:
                current_iteration += 1
                print(
                    f"Grid search progress: {current_iteration}/{total_iterations} combinations evaluated",
                    flush=True,
                )
        ...
        print(
            f"Best hyperparameters from grid search: {best_params}, val_acc={best_val_acc:.4f}"
        )
        return best_params
